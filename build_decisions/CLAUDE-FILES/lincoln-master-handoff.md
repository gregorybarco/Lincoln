...
No Docker. Ever.
No cloud exposure. All inference via localhost:11434. All data in local SQLite + ChromaDB.
Everything configurable via Settings UI. No hardcoding in Python files or .env. All settings in SQLite _DEFAULT_SETTINGS table, exposed in Settings panel, read at runtime. Violations: editing __init__.py for version (banned), editing .env directly for keys (banned), hardcoding paths in Python (banned).
No hardcoded model names. LLM switchable from UI per session. Embed model fixed in .env.
Aider always suggestion-only. Never auto-commits. User reviews all proposed changes.
ASCII-127 Law. All Fortran and Python source files must stay within 7-bit ASCII (0-127). Unicode causes silent encoding corruption at NTFS/WSL2 boundary and fatal crackfortran crashes.
Purpose over theatre. No arbitrary limits, no fake safety theatre.
5. BANNED CODING PATTERNS (CRITICAL — READ BEFORE PATCHING)
Python
NEVER mix yield and return value in the same function. If a function has any yield it is a generator. return value silently discards the value. Fix: yield (value_tuple) then bare return. Caller checks isinstance(item, tuple).
NEVER assume a JS/CSS/HTML file was redeployed just because the UI shows a new version. Check with Select-String on disk. Flask serves static files fresh but Python files load once at process start — .py changes require full restart (Ctrl+C + relaunch).
NEVER trust an empty devtools console as proof nothing is wrong. Swallowed exceptions (except: pass) produce zero errors. Add console.error/logging to all catch-all blocks during debugging.
marked.js v9 renderer: renderer.code receives a single token object {type, raw, lang, text, escaped} — NOT the old three positional args. Lincoln pins marked@9.1.6 in CDN.
Tool Call Schema (Ollama payload — causes silent 400 errors if violated)
tool_calls arguments MUST be native Python dicts. NEVER json.dumps(arguments). This was the root cause of the 400 Bad Request after save_memory fired.
Tool result messages MUST use "role": "tool". Never disguise as "role": "user".
Every assistant tool-call message MUST have "id": tool_call_id and "type": "function" alongside the function dict.
Every tool result message MUST have "tool_call_id": tool_call_id linking it back to the assistant's request.
Never inject meta-prompting into tool result content. Tool responses = raw data only. Guidance goes in the system prompt.
...
Lincoln persona — identity, formatting rules, autonomous multi-step directive
Lincoln Life cycle — agent phases, behavioral silos
Memory Save Behavior — when and how to call save_memory proactively
Tool Chaining Mandate — after web_search always call read_url; dedup before saving memory
...
Model routing (specialist models per task type)
True multi-agent (parallel agents, shared memory pool)
...
9. LONG-TERM GOALS
QLoRA fine-tuning on RTX 5060 Ti 16GB — fine-tune Qwen or Gemma on OptionsPricing + BARCO_CORE data for domain-specific financial modelling. MLflow experiment tracking prerequisite.
Lincoln self-maintenance — LLM has read access to own source via RAG project; proposes patches via Aider suggestion mode; user reviews and approves.
Bloomberg/Schwab terminal integration — OCR pipeline operational; future: structured data extraction from OVDV, option chains, Greeks tables into pandas DataFrames stored locally.
Agentic web browsing — Playwright/Selenium via WSL for job application automation, web form filling, data collection.
Commercialisation path — private enterprise AI, financial research tools, secure multimodal extraction.
...
Call recent_chats to pull last 5-10 sessions.
Read all userMemories entries in full.
Read this document if it is in the project files.
Cross-check — identify anything in recent chats NOT yet captured and add it.
Confirm to the user: "I have reviewed memory entries 1-N, recent chats, and the handoff document. Here is what I know is pending..." then list current priority queue.
Only THEN begin any build, patch, or planning work.
